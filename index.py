...
def status_thread():
    while True:
        time.sleep(5)
        socketio.emit('status', StorjAPI.getNodeStatus(), namespace='/ws-metadisk')

@socketio.on('connect', namespace='/ws-metadisk')
...
@socketio.on('disconnect', namespace='/ws-metadisk')
